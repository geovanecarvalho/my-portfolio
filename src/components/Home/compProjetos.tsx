      ...
      <div className="w-full flex flex-col items-center max-w-7xl mx-auto bg-gradient-to-r from-blue-900 via-blue-700 to-blue-900 p-8 rounded-3xl shadow-2xl border-2 border-blue-400">
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {projetos.map((p) => (
            <div
              key={p.id}
              className="bg-white/10 backdrop-blur-md rounded-2xl shadow-lg border-2 border-white/30 p-6 flex flex-col items-center min-h-[340px]"
            >
              {/* Imagem do projeto centralizada e redonda */}
              {p.imagemCapa && (
                <img
                  src={p.imagemCapa}
                  alt={p.nome}
                  className="w-24 h-24 rounded-full object-cover border-4 border-blue-300 shadow mb-2"
                />
              )}

              {/* Título centralizado */}
              <h5 className="text-lg font-bold text-white mb-2 text-center drop-shadow">{p.nome}</h5>

              {/* Dados do projeto alinhados à esquerda em fieldset */}
              <fieldset className="border-2 border-white rounded-xl px-4 py-3 bg-white/10 w-full mb-2">
                ...
                <div className="text-white text-sm mt-2 space-y-1 text-left">
                  <div>
                    ...
                    <span
                      ...
                      style={{ textIndent: "1rem", marginBottom: "0.75rem" }} // 4 espaços ≈ 1rem
                    >
                      {p.descricao}
                    </span>
                  </div>
              ...
              {p.gifPreview && (
                <div className="w-full flex justify-center">
                  <img
                    src={p.gifPreview}
                    alt={p.nome + ' preview'}
                    className="w-full max-w-xs h-48 rounded-lg object-cover border-2 border-blue-200 shadow my-2"
                  />
                </div>
              )}

              {/* Fieldset para links com ícones */}
              <fieldset className="border-2 border-white rounded-xl px-4 py-3 bg-white/10 w-full mb-2 mt-2">
                <legend className="font-semibold text-white px-2 flex items-center gap-2">
                  Links
                </legend>
